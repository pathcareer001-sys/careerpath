...
export default function StatCard({ title, value, gradient = "blue" }: Props) {
  return (
    <div className={`group relative overflow-hidden rounded-xl border border-[#E2E8F0] bg-white p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg ${lightBgs[gradient]}`}>
      <div className="relative z-10">
        <p className="text-[28px] font-medium text-[#0F172A] leading-none">{value}</p>
        <p className="mt-1.5 text-[13px] text-[#64748B] font-medium">{title}</p>
      </div>
