...
export default function CreateCompanyDialog({ company, open, onClose }: Props) {
  ...
  const handleSave = async () => {
    try {
      ...
      toast.error(isNew ? "Failed to create company" : "Failed to update company");
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto p-6">
        <h2 className="text-xl font-medium mb-6">{isNew ? "Create Company" : "Edit Company"}</h2>
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-4 border-t">
            ...
            <AppButton onClick={handleSave} disabled={updateCompany.isPending || createCompany.isPending}>
              {updateCompany.isPending || createCompany.isPending ? "Saving..." : "Save"}
            </AppButton>
          </div>
        </div>
      </DialogContent>
