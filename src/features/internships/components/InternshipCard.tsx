...
export default function InternshipCard({ internship }: { internship: Internship }) {
  ...
  const tagClass = tagStyles[internship.type] || "bg-gradient-to-r from-blue-50 to-indigo-50 text-blue-700 border border-blue-200";

  return (
    <Link to={`/internships/${internship.id}`} className="block group animate-fade-in-up">
      <div className="relative bg-white border border-[#E2E8F0] rounded-xl p-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-blue-500/5 hover:border-blue-200">
        <button
          onClick={handleBookmark}
          className="absolute top-4 right-4 transition-colors duration-200"
        >
          {isBookmarked ? (
            <BookmarkCheck size="16" className="text-[#2563EB]" />
          ) : (
            <Bookmark size="16" className="text-[#CBD5E1] group-hover:text-[#2563EB]" />
          )}
        </button>
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-medium text-[#0F172A] leading-snug group-hover:text-[#2563EB] transition-colors duration-200">{internship.title}</h3>
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-[#F1F5F9]">
          ...
          <span className="text-[13px] font-medium text-[#2563EB] opacity-0 -translate-x-2 transition-all duration-200 group-hover:opacity-100 group-hover:translate-x-0">
            View Details →
          </span>
        </div>
      </div>
